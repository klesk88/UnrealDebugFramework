...
private:
    [[nodiscard]] FKLDebugImGuiServerObjectFeatures& GetOrAddFeaturesPerObject(const UWorld& _World, const FNetworkGUID& _NetworkID);
    [[nodiscard]] FKLDebugImGuiServerObjectFeatures* TryGetFeaturesPerObjectMutable(const FNetworkGUID& _NetworkID);
    void RemoveObjectFeatures(const FNetworkGUID& _NetworkID);
    [[nodiscard]] TOptional<KL::Debug::ImGui::Features::Types::FeatureIndex> GetFeatureIndex(const FKLDebugImGuiFeatureContainerBase& _Container, const KL::Debug::ImGui::Features::Types::FeatureIndex _FeatureIndex, const FName& _FeatureNameID) const;
    ...
    [[nodiscard]] bool Rcv_HandleClientFeatureStatusUpdate(const FKLDebugImGuiFeaturesTypesContainerManager& _FeatureContainerManager, const UWorld& _World, FArchive& _Archive);
    [[nodiscard]] bool Rcv_HandleClientFeatureRequestUpdate(const FKLDebugImGuiFeaturesTypesContainerManager& _FeatureContainerManager, const UWorld& _World, FKLDebugImGuiNetworkingMessage_FeatureRequestUpdate& _Message, FArchive& _ArchiveWriter);
    [[nodiscard]] bool Rcv_WindowToogle(const FKLDebugNetworkingMessage_WindowToogle& _Message, const UWorld& _World, FKLDebugFrameworkModeManager& _ModeManager, FKLDebugFrameworkBottomBarManager& _BarManager, FArchive& _Archive);
    [[nodiscard]] bool Rcv_WindowRequestUpdate(const UWorld& _World, FKLDebugNetworkingMessage_WindowRequestUpdate& _Message, FKLDebugFrameworkModeManager& _ModeManager, FKLDebugFrameworkBottomBarManager& _BarManager, FArchive& _Archive);
    [[nodiscard]] bool Recv_SelectableUpdate(const UWorld& _World, const FKLDebugImGuiFeatureContainerBase& _Container, const FKLDebugImGuiNetworkingMessage_FeatureStatusUpdate& _Update);
    [[nodiscard]] bool Recv_UniqueUpdate(const UWorld& _World, const FKLDebugImGuiFeatureContainerBase& _Container, const FKLDebugImGuiNetworkingMessage_FeatureStatusUpdate& _Update);

    [[nodiscard]] bool CanAddFeatureToTickList(const IKLDebugFeatureInterfaceBase& _Feature) const;
