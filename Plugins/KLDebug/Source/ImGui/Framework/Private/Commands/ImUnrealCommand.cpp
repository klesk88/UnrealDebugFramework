...
namespace ImUnrealCommand
{
    ...
    struct Preset
    {
        ...
        {
            FString mName;
            TArray<FString> mCommands;
            TArray<FString> mFilters;
        };

        Preset(const FString& name);
        void AddCommands(const TArray<FString>& commands)
        {
            mConfig.mCommands.Append(commands);
            mDirty = true;
        }
        void AddFilters(const TArray<FString>& filters)
        {
            mConfig.mFilters.Append(filters);
            mDirty = true;
        }
        void Update();
    ...
    struct FilterConfig
    {
        enum : uint32
        {
            kAllTypeMask = (1 << static_cast<uint32>(PresetEntry::eType::_Count)) - 1
        };
    ...
    PresetEntry::PresetEntry(const FString& command, IConsoleObject* pConsoleObj)
    {
        ...
        if (mpCommandObj)
        {
            if (mpCommandObj->IsVariableBool())
                mType = eType::Bool;
            else if (mpCommandObj->IsVariableInt())
                mType = eType::Int;
            else if (mpCommandObj->IsVariableFloat())
                mType = eType::Float;
            else if (mpCommandObj->IsVariableString())
                mType = eType::String;
            else if (mpCommandObj->AsCommand())
                mType = eType::Command;
            else
            {
    ...
    void Preset::Update()
    {
        ...
        if (mSortResults)
        {
            mEntries.Sort([](const PresetEntry& A, const PresetEntry& B) {
                return A.mCommand < B.mCommand;
            });
        }
    }
    ...
    void Show(CommandContext* pCmdContext)
    {
        ...
        if (pCmdContext->mWindowActive)
        {
            ...
            if (ImGui::Begin("Unreal Console Commands [?]", &pCmdContext->mWindowActive))
            {
                ...
                {
                    ImGui::SetTooltip(
                        "[ Unreal Console Commands browser ]\n"
                        "By Sammy Fatnassi.\n"
